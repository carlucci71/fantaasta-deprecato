...
public interface FantaroseRepository extends CrudRepository<Fantarose, Integer> {
	...
	Iterable<SpesoPerRuolo> spesoPerAllenatore();
	...
	@Query("SELECT  new com.daniele.asta.dto.GiocatoriPerSquadra(a.nome as allenatore,g.squadra,g.ruolo,g.nome as giocatore,f.costo,f.sqlTime) from Fantarose f,Giocatori g,Allenatori a where g.id = idGiocatore and a.id = idAllenatore order by allenatore,ruolo desc,giocatore")
